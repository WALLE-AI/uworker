use agentrs::provider::LlmProvider;
use agentrs::provider::compat::ProviderCompat;
use agentrs::provider::debug::DebugConfig;
use agentrs::provider::openai::OpenAIProvider;
use agentrs::types::llm::{LlmEvent, LlmRequest};
use agentrs::types::message::{ContentBlock, Message, Role, StopReason};
use serde_json::json;
...
async fn test_openai_api_error_non_success_status() {
    ...
    match result.unwrap_err() {
        agentrs::provider::ProviderError::Api { status, .. } => {
...
async fn test_openai_rate_limited() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        agentrs::provider::ProviderError::RateLimited { retry_after_ms } => {
