...
- **Local tool suite** — Read, Write, Edit, ExecCommand, Grep, Glob, ViewImage, TodoWrite (or a dependency-aware task graph), and Spawn (sub-agents), plus Skills, plan mode, and deferred tool discovery
...
./target/release/agentrs config init
# Edit the generated config (run `agentrs config path` to find it)

# Single-shot mode
agentrs "Read Cargo.toml and explain the dependencies"
...
```bash
agentrs --max-turns 10 "Run the task"
agentrs --max-tool-call-malformed-turns 2 "Run the task"
agentrs --max-tool-call-failure-turns 2 "Run the task"
```
