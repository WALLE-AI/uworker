# agentrs JSON Stream Protocol Spec

> This protocol defines the communication between agentrs (Rust CLI) and a host client (e.g., AgentrsUi Electron app) via stdin/stdout JSON Lines.
...
- **Encoding**: UTF-8
- **Activation**: `agentrs --json-stream [other flags]`
...
agentrs --json-stream --session-id my-conv-123 --provider openai --model gpt-4o

# Resume an existing session
agentrs --json-stream --resume my-conv-123 --provider openai --model gpt-4o

# Fork an existing session into a new id (read it from `ready.session_id`)
agentrs --json-stream --resume my-conv-123 --fork-session --provider openai --model gpt-4o
```
