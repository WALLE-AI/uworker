...
```bash
agentrs --profile claude-fast "Quick question"
agentrs --profile claude-deep "Deep security audit"
agentrs --profile dev "Create a GitHub issue"
```
...
```bash
agentrs --profile deepseek-v4-pro --thinking enabled
```
...
3. Credentials are saved alongside the global config (run `agentrs config path` to find the directory)
