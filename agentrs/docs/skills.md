...
2. Substitutes variables (`$ARGUMENTS`, `$0`, `${AGENTRS_SKILL_DIR}`)
...
|----------|------|-------------|
| 1 | `.agentrs/skills/` | Project-local skills (checked-in with the repo) |
| 2 | `<CONFIG_DIR>/agentrs/skills/` | User-global skills (see below) |
| 3 | `.agentrs/commands/` | Legacy flat `.md` files (backward compatibility) |
...
>
> Run `agentrs skills path` to see the actual paths on your machine.
...
| `${AGENTRS_SKILL_DIR}` | Absolute path to the directory containing this skill's `SKILL.md` |
...
Working directory: ${AGENTRS_SKILL_DIR}
```
...
Use `agentrs skills path` to see which directories are being scanned and whether
they exist on disk:

```
$ agentrs skills path
User:    ~/Library/Application Support/agentrs/skills  (exists)
Project: /path/to/repo/.agentrs/skills                 (exists)
Legacy:  /path/to/repo/.agentrs/commands                (not found)
```
