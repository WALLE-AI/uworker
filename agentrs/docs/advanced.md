...
command = "echo \"$(date): ${TOOL_INPUT_COMMAND}\" >> .agentrs/audit.log"
...
```bash
# CLI — logs to /tmp/agentrs-logs at debug level
agentrs --log-dir /tmp/agentrs-logs --log-level debug "Read Cargo.toml"
```
...
| `"agentrs_providers=debug,info"` | Debug for providers, info for everything else |
...
| Platform | Path |
|----------|------|
| macOS | `~/Library/Logs/agentrs/` |
| Linux | `$XDG_STATE_HOME/agentrs/logs/` or `~/.local/state/agentrs/logs/` |
| Windows | `{data_local_dir}/agentrs/logs/` |
...
```json
{"timestamp":"2026-05-13T12:12:52.431Z","level":"INFO","fields":{"message":"mcp server connected","server":"sentry","tools":20},"target":"agentrs_mcp","spans":[{"name":"agent_run","session_id":"abc-123","msg_id":"msg-456"}]}
```
...
| Field | Description |
|-------|-------------|
| `target` | Source crate (`agentrs_agent`, `agentrs_providers`, `agentrs_mcp`, etc.) |
...
grep '"session_id":"abc-123"' 2026-05-13.agentrs.log | jq .
```

### Library Integration

When agentrs is used as a library (e.g. embedded in a backend server), the `create_file_layer()` API provides a composable tracing layer:

```rust
use agentrs_config::logging::{ResolvedLogging, create_file_layer};

let resolved = ResolvedLogging {
    enabled: true,
    level: "agentrs_agent=debug,agentrs_providers=debug".to_string(),
...
tracing_subscriber::registry()
    .with(your_app_layer)
    .with(layer)  // agentrs logs → separate agentrs.log file
    .init();
```

The host application owns the global subscriber; agentrs library crates only emit tracing events and never initialize a subscriber themselves.
...
1. **Global**: `<config_dir>/agentrs/AGENTS.md` — user-level instructions for all projects
...
Running agentrs in `packages/server/` produces a system prompt containing both files, workspace first, then server.
...
```
<config_dir>/agentrs/projects/<sanitized-project-path>/memory/
...
```bash
export AGENTRS_MEMORY_DIR=/custom/path
```
...
plan_directory = ".agentrs/plans"  # Where plan files are saved
```
