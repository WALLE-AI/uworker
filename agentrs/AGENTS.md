...
Rules and conventions for AI assistants and contributors working on agentrs.
...
built-in tools (Read, Write, Edit, ExecCommand, Grep, Glob, ViewImage, TodoWrite or the
Task graph, WebFetch, WebSearch, Spawn), supports MCP servers, skills, hooks, and long-term memory. It
...
|-------|-------|----------------|
| Bottom | `agentrs-types` | Shared provider-neutral data types (LLM, message, tool) — zero internal deps |
| Bottom | `agentrs-compact` | Context compression algorithms (folding, sanitization, tokenization) |
...
| Mid | `agentrs-tools` | Local tools (Read, Write, Edit, ExecCommand, Grep, Glob, ViewImage, TodoWrite, Task graph, ToolSearch) and network tools (WebFetch, WebSearch) |
...
| Top | `agentrs-agent` | Agent engine, session management, orchestration, and agent-level tools such as Spawn |
| Top | `agentrs-cli` | CLI binary entry point |
...
- AgentrsCLI is the built-in SDK/library integration path. Do not model it as an
  external agent subprocess when changing Agentrs host integration code.
...
- All shell invocations must go through `agentrs_config::shell` module
...
| [json-stream-protocol.md](docs/json-stream-protocol.md) | JSON Lines protocol spec for host integration (e.g. AgentrsUI) |
