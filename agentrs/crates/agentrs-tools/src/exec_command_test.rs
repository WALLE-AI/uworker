...
mod tests {
    ...
    async fn execute_injects_runtime_env() {
        let tool = ExecCommandTool::new_with_env(
            std::env::temp_dir(),
            vec![("AGENTRS_RUNTIME_ENV_TEST".to_string(), "exec-env-value".to_string())],
        );
        #[cfg(windows)]
        let input = json!({"cmd": "Write-Output $env:AGENTRS_RUNTIME_ENV_TEST", "shell": "powershell"});
        #[cfg(not(windows))]
        let input = json!({"cmd": "printf '%s' \"$AGENTRS_RUNTIME_ENV_TEST\"", "shell": "sh"});
    ...
    async fn execute_timeout_preserves_stdout_emitted_before_timeout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        #[cfg(windows)]
        let cmd = "Write-Output agentrs_stdout_before_timeout; Start-Sleep -Seconds 5";
        #[cfg(not(windows))]
        let cmd = "printf 'agentrs_stdout_before_timeout\\n'; sleep 5";
        let input = json!({
            "cmd": cmd,
            "timeout": 1500
        });
        ...
        assert!(
            result.content.contains("STDOUT:\n") && result.content.contains("agentrs_stdout_before_timeout"),
    ...
    async fn execute_timeout_preserves_stderr_emitted_before_timeout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        #[cfg(windows)]
        let cmd = "[Console]::Error.WriteLine('agentrs_stderr_before_timeout'); Start-Sleep -Seconds 5";
        #[cfg(not(windows))]
        let cmd = "printf 'agentrs_stderr_before_timeout\\n' >&2; sleep 5";
        let input = json!({
            "cmd": cmd,
            "timeout": 1500
        });
        ...
        assert!(
            result.content.contains("STDERR:\n") && result.content.contains("agentrs_stderr_before_timeout"),
    ...
    async fn execute_timeout_omits_output_after_timeout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        #[cfg(windows)]
        let cmd = "Write-Output agentrs_before_timeout; Start-Sleep -Seconds 5; Write-Output agentrs_after_timeout";
        #[cfg(not(windows))]
        let cmd = "printf 'agentrs_before_timeout\\n'; sleep 5; printf 'agentrs_after_timeout\\n'";
        let input = json!({
            "cmd": cmd,
            "timeout": 1500
        });
        ...
        assert!(
            result.content.contains("agentrs_before_timeout"),
            "output emitted before timeout should be preserved, got: {}",
            result.content
        );
        assert!(
            !result.content.contains("agentrs_after_timeout"),
    ...
    async fn execute_powershell_write_output_returns_stdout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        let input = json!({
            "cmd": "Write-Output agentrs_powershell_stdout_probe",
            "shell": "powershell"
        });
        ...
        assert!(!result.is_error, "unexpected error: {}", result.content);
        assert!(
            result.content.contains("STDOUT:\n") && result.content.contains("agentrs_powershell_stdout_probe"),
    ...
    async fn execute_cmd_echo_returns_stdout() {
        let tool = ExecCommandTool::new(std::env::temp_dir());
        let input = json!({
            "cmd": "echo agentrs_cmd_stdout_probe",
            "shell": "cmd"
        });
        ...
        assert!(!result.is_error, "unexpected error: {}", result.content);
        assert!(
            result.content.contains("STDOUT:\n") && result.content.contains("agentrs_cmd_stdout_probe"),
