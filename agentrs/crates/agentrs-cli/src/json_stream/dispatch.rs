...
use agentrs_protocol::ToolApprovalResult;
...
pub(super) fn handle(cmd: ProtocolCommand, engine: &mut AgentEngine, ctx: &StreamContext) -> DispatchOutcome {
    match cmd {
        ...
        ProtocolCommand::InitHistory { text } => {
            tracing::debug!(target: "agentrs_protocol", chars = text.len(), "InitHistory received");
        }
        ProtocolCommand::SetMode { mode } => {
            ...
            tracing::debug!(target: "agentrs_protocol", mode = %mode_str, "SetMode applied");
        }
