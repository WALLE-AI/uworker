...
use agentrs_agent::engine::AgentEngine;
use agentrs_agent::output::OutputSink;
use agentrs_config::config::{McpServerConfig, TransportType};
use agentrs_mcp::manager::McpManager;
use agentrs_mcp::tool_proxy::register_single_server_tools;
use agentrs_protocol::commands::ProtocolCommand;
use agentrs_protocol::events::ProtocolEvent;
    ...
    while let Some(cmd) = cmd_rx.recv().await {
        match cmd {
            ProtocolCommand::AddMcpServer {
                ...
                tracing::info!(target: "agentrs_mcp", %name, %transport, ?command, "AddMcpServer received");
                ...
                tracing::info!(target: "agentrs_mcp", %name, "connecting to mcp server");
                match McpManager::connect_all(&single_configs).await {
                    Ok(mgr) => {
                        ...
                        tracing::info!(target: "agentrs_mcp", %name, tools = tool_names.len(), "mcp server connected");
                    ...
                    Err(e) => {
                        tracing::warn!(target: "agentrs_mcp", %name, error = %e, "mcp server connection failed");
