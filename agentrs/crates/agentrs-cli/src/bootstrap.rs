//! Shared initialization steps for agentrs-cli entry points.
//!
...
//!
//! Note: `agentrs_agent` also exposes a module named `bootstrap`
//! (`agentrs_agent::bootstrap::AgentBootstrap`). Call sites should keep using
//! that fully-qualified path for the engine builder itself; this module is
//! `crate::bootstrap` and only wraps it.

use std::sync::Arc;

use agentrs_agent::bootstrap::{AgentBootstrap, BootstrapResult};
use agentrs_agent::output::OutputSink;
use agentrs_agent::session::{Session, SessionManager};
use agentrs_compact::CompactLevel;
use agentrs_config::config::{CliArgs, Config};
use agentrs_config::logging::{LoggingGuard, create_file_layer};
