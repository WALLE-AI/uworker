use agentrs_config::compat::{self, ProviderCompat, ToolWireShape};
use agentrs_config::schema::legalize_json_schema;
use agentrs_types::llm::{LlmRequest, ThinkingConfig};
use agentrs_types::tool::{ToolDef, truncate_deferred_description};
...
impl AnthropicWireProjector {
    ...
    pub(crate) fn project(
        ...
        apply_extra_body(params.provider, &mut body, compat);
...
impl OpenAiProjector {
    ...
    pub(crate) fn project(request: &LlmRequest, compat: &ProviderCompat) -> Result<Value, ProjectionError> {
        ...
        apply_extra_body(WireProvider::OpenAi, &mut body, compat);
...
/// Merge `compat.extra_body` into a projected request body.
///
/// Projector-produced keys win: a vendor field can only add to the wire
/// contract, never rewrite what the neutral projection already decided.
pub(crate) fn apply_extra_body(provider: WireProvider, body: &mut Value, compat: &ProviderCompat) {
    let Some(extra) = compat.extra_body() else {
        return;
    };
    ...
    for (key, value) in extra {
        if target.contains_key(key) {
            tracing::warn!(
                ...
                %provider,
                field = %key,
                "compat.extra_body field skipped because the projector already set it"
            );
            continue;
        }
