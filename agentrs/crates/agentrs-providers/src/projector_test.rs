...
mod tests {
    use super::*;
    use agentrs_config::compat::ModelMaxTokensRule;
    ...
    #[test]
    fn extra_body_adds_vendor_fields_to_the_openai_request() {
        let mut compat = ProviderCompat::openai_defaults();
        let mut extra = serde_json::Map::new();
        extra.insert("chat_template_kwargs".to_string(), json!({ "enable_thinking": true }));
        compat.transport.extra_body = Some(extra);

        let body =
            OpenAiProjector::project(&test_request(Vec::new(), None), &compat).expect("projection should succeed");

        assert_eq!(body["chat_template_kwargs"]["enable_thinking"], json!(true));
        // Neutral projection must be untouched by the passthrough.
        assert_eq!(body["model"], json!("test-model"));
        assert_eq!(body["stream"], json!(true));
    }

    #[test]
    fn extra_body_cannot_overwrite_projector_produced_fields() {
        let mut compat = ProviderCompat::openai_defaults();
        let mut extra = serde_json::Map::new();
    ...
    #[test]
    fn extra_body_also_reaches_anthropic_wire_requests() {
        let mut compat = ProviderCompat::anthropic_defaults();
        let mut extra = serde_json::Map::new();
        extra.insert("metadata".to_string(), json!({ "user_id": "u-1" }));
        compat.transport.extra_body = Some(extra);
        ...
        let body = AnthropicWireProjector::project(&test_request(Vec::new(), None), &compat, params)
