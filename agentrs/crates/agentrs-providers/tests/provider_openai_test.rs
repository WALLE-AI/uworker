...
use agentrs_config::compat::{OpenAiApiMode, ProviderCompat};
use agentrs_providers::LlmProvider;
use agentrs_providers::openai::OpenAIProvider;
use agentrs_types::llm::{LlmEvent, LlmRequest};
use agentrs_types::message::{ContentBlock, Message, Role, StopReason};
...
async fn test_openai_api_error_non_success_status() {
    ...
    match result.unwrap_err() {
        agentrs_providers::ProviderError::Api { status, .. } => {
...
async fn test_aio_140_openai_tools_wire_shape_mismatch_error_is_readable_and_not_retried() {
    ...
    match result.unwrap_err() {
        agentrs_providers::ProviderError::Api { status, message } => {
...
async fn test_openai_rate_limited() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        agentrs_providers::ProviderError::RateLimited { retry_after_ms, body } => {
