...
///
/// Path: `$TMPDIR/agentrs-bundled-skills-{pid}/{skill_name}`
...
pub fn get_bundled_skill_extract_dir(skill_name: &str) -> PathBuf {
    ...
    tmp.join(format!("agentrs-bundled-skills-{pid}")).join(skill_name)
}
...
pub async fn extract_bundled_skill_files(skill_name: &str, files: &[(&str, &str)]) -> Option<PathBuf> {
    ...
    match write_skill_files(&dir, files).await {
        ...
        Err(e) => {
            ...
            tracing::warn!(target: "agentrs_skills", skill = %skill_name, path = %dir.display(), error = %e, "failed to extract bundled skill");
            None
        }
    }
}
