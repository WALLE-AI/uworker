...
async fn load_skill_file(
    ...
    // used for ${AGENTRS_SKILL_DIR} variable substitution in skill content.
...
///
/// Examples:
/// - base=`<config_dir>/agentrs/skills`, target=`<config_dir>/agentrs/skills/db/migrate` → `"db:migrate"`
/// - base=`<config_dir>/agentrs/skills`, target=`<config_dir>/agentrs/skills/my-skill` → `"my-skill"`
