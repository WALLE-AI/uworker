            ...
            for &file_path in file_paths {
                ...
                for pattern in &entry.patterns {
                    if pattern.matches(&rel) {
                        tracing::info!(target: "agentrs_skills", skill = %name, path = %rel, "activated conditional skill");
...
fn compile_patterns(skill_name: &str, raw_patterns: &[String]) -> Vec<Pattern> {
    raw_patterns
        ...
        .filter_map(|p| match Pattern::new(p) {
            Ok(pat) => Some(pat),
            Err(e) => {
                tracing::warn!(target: "agentrs_skills", skill = %skill_name, pattern = %p, error = %e, "invalid glob pattern, skipping");
                None
            }
        })
        .collect()
