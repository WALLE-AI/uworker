...
use agentrs_config::config::app_config_dir;

// ---------------------------------------------------------------------------
// User-level directories (<config_dir>/agentrs/)
// ---------------------------------------------------------------------------

/// Return the user-level skills directory: `<config_dir>/agentrs/skills/`
///
...
/// Return the user-level legacy commands directory: `<config_dir>/agentrs/commands/`
...
/// Find all project-level `.agentrs/skills/` directories by walking up from
...
/// Find all project-level `.agentrs/commands/` directories (legacy), same walk.
...
///
/// Each path in `add_dirs` is checked for a `.agentrs/skills/` subdirectory.
/// Only directories that exist are included.
pub fn additional_skills_dirs(add_dirs: &[PathBuf]) -> Vec<PathBuf> {
    add_dirs
        .iter()
        .map(|d| d.join(".agentrs").join("skills"))
        .filter(|p| p.is_dir())
        .collect()
}
...
/// `.agentrs/<subdir>/` directories that exist. Returns deepest-first.
fn walk_up_dirs(cwd: &Path, subdir: &str) -> Vec<PathBuf> {
    ...
    loop {
        let candidate = current.join(".agentrs").join(subdir);
        if candidate.is_dir() {
            dirs.push(candidate);
        }
