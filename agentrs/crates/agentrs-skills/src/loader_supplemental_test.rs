...
async fn tc_11_1_bare_mode_only_loads_add_dirs() {
    ...
    // Put a skill in add_dir's .agentrs/skills/
    let add_skills_dir = add_tmp.path().join(".agentrs").join("skills");
...
async fn tc_11_5_empty_scenario_returns_empty_vec() {
    ...
    let tmp = TempDir::new().unwrap();
    // tmp exists but has no .agentrs/skills
...
// matching TS skillRoot used for ${AGENTRS_SKILL_DIR} substitution.
// -----------------------------------------------------------------------
