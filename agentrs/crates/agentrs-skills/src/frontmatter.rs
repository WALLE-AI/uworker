...
fn parse_yaml_with_fallback(yaml_text: &str) -> FrontmatterData {
    ...
    match serde_yaml::from_str::<FrontmatterData>(yaml_text) {
        Ok(data) => return data,
        Err(e) => {
            tracing::warn!(target: "agentrs_skills", error = %e, "frontmatter first-pass parse failed");
        }
    }
    ...
    match serde_yaml::from_str::<FrontmatterData>(&fixed) {
        Ok(data) => data,
        Err(e) => {
            tracing::warn!(target: "agentrs_skills", error = %e, "frontmatter second-pass parse failed, returning empty");
