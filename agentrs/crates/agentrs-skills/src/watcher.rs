...
impl SkillWatcher {
    ...
    pub fn watch_directory(&mut self, dir: &Path) -> notify::Result<()> {
        if !dir.is_dir() {
            tracing::debug!(target: "agentrs_skills", path = %dir.display(), "skipped non-existent watcher directory");
            return Ok(());
        }
        ...
        if let Some(ref mut w) = self.watcher {
            ...
            tracing::debug!(target: "agentrs_skills", path = %dir.display(), "watching skill directory");
        }
...
fn should_ignore(event: &Event) -> bool {
    ...
    // directory components — otherwise paths like `.agentrs/skills/SKILL.md`
    // would be incorrectly filtered because `.agentrs` starts with `.`.
