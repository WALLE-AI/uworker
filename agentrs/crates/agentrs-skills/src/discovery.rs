...
/// Manages runtime discovery of `.agentrs/skills/` directories found in
...
impl RuntimeDiscovery {
    ...
    /// Discover `.agentrs/skills/` directories by walking up from each file path to `cwd`.
    ///
    ...
    pub async fn discover_dirs_for_paths(&mut self, file_paths: &[&str], cwd: &str) -> Vec<PathBuf> {
        ...
        for &file_path in file_paths {
            ...
            loop {
                ...
                let skill_dir = current.join(".agentrs").join("skills");
                ...
                if !self.checked_dirs.contains(&skill_dir) {
                    ...
                    if tokio::fs::metadata(&skill_dir).await.is_ok() {
                        ...
                        if is_path_gitignored(containing_dir, resolved_cwd).await {
                            tracing::debug!(target: "agentrs_skills", path = %skill_dir.display(), "skipping gitignored skills directory");
                        } else {
    ...
    pub async fn add_skill_directories(&mut self, dirs: &[PathBuf]) -> usize {
        ...
        if added > 0 {
            tracing::info!(target: "agentrs_skills", added, directories = dirs.len(), "dynamically discovered new skills");
        }
    ...
    /// already known not to contain a `.agentrs/skills/` subdirectory.
