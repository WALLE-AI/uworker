...
mod discovery_supplemental_tests {
    ...
    /// Create a `.agentrs/skills/` directory inside `parent`.
    fn create_skill_dir(parent: &Path) -> PathBuf {
        let dir = parent.join(".agentrs").join("skills");
    ...
    // TC-22: discovers `.agentrs/skills/` inside a direct subdirectory of cwd.
    #[tokio::test]
    async fn tc22_discover_dirs_finds_agentrs_skills_in_subdir() {
        let tmp = TempDir::new().unwrap();
        let cwd = tmp.path().to_str().unwrap().to_string();

        // Create /tmp/proj/module/.agentrs/skills/
        ...
        assert!(found[0].ends_with(".agentrs/skills"));
    }
    ...
    // TC-23: `.agentrs/skills/` at cwd level is not returned (loaded at startup).
    #[tokio::test]
    ...
    // TC-25: directories without `.agentrs/skills/` are still recorded to avoid
    // repeated stat calls.
    #[tokio::test]
    async fn tc25_discover_dirs_records_miss_dirs_in_checked() {
        ...
        // `b` does NOT have .agentrs/skills/
