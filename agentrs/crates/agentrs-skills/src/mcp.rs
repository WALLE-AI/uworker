...
pub async fn load_mcp_skills(manager: &McpManager) -> Vec<LoadedSkill> {
    ...
    for server_name in manager.server_names() {
        ...
        let resources = match manager.list_resources(&server_name).await {
            Ok(r) => r,
            Err(e) => {
                tracing::warn!(target: "agentrs_skills", server = %server_name, error = %e, "failed to list mcp resources");
                continue;
            }
        };
        ...
        for resource in resources {
            ...
            let text = match manager.read_resource(&server_name, &resource.uri).await {
                Ok(t) => t,
                Err(e) => {
                    tracing::warn!(target: "agentrs_skills", server = %server_name, uri = %resource.uri, error = %e, "failed to read mcp resource");
                    continue;
                }
            };
