...
mod tests {
    ...
    #[test]
    fn test_agentrs_skill_dir_substitution() {
        let result = substitute_arguments("dir=${AGENTRS_SKILL_DIR}", None, &[], Some("/my/skill"), None);
        assert_eq!(result, "dir=/my/skill");
    }

    #[test]
    fn test_agentrs_session_id_substitution() {
        let result = substitute_arguments("sid=${AGENTRS_SESSION_ID}", None, &[], None, Some("sess-123"));
...
mod supplemental_tests {
    ...
    // -----------------------------------------------------------------------
    // TC-6.x: ${AGENTRS_SKILL_DIR} substitution
    // -----------------------------------------------------------------------

    #[test]
    fn tc_6_1_skill_dir_replaced() {
        let r = substitute_arguments(
            "cd ${AGENTRS_SKILL_DIR}",
            None,
            &[],
            Some("/home/user/.agentrs/skills/my-skill"),
            None,
        );
        assert_eq!(r, "cd /home/user/.agentrs/skills/my-skill");
    }

    #[test]
    fn tc_6_2_skill_dir_none_not_replaced() {
        // skill_root = None → ${AGENTRS_SKILL_DIR} stays unreplaced
        let r = substitute_arguments("cd ${AGENTRS_SKILL_DIR}", None, &[], None, None);
        assert_eq!(r, "cd ${AGENTRS_SKILL_DIR}");
    }

    #[test]
    fn tc_6_3_skill_dir_multiple_occurrences() {
        let r = substitute_arguments(
            "${AGENTRS_SKILL_DIR}/a and ${AGENTRS_SKILL_DIR}/b",
            None,
            &[],
            Some("/skills/foo"),
            None,
    ...
    // -----------------------------------------------------------------------
    // TC-7.x: ${AGENTRS_SESSION_ID} substitution
    // -----------------------------------------------------------------------

    #[test]
    fn tc_7_1_session_id_replaced() {
        let r = substitute_arguments("Session: ${AGENTRS_SESSION_ID}", None, &[], None, Some("abc-123"));
        assert_eq!(r, "Session: abc-123");
    }

    #[test]
    fn tc_7_2_session_id_none_not_replaced() {
        let r = substitute_arguments("Session: ${AGENTRS_SESSION_ID}", None, &[], None, None);
        assert_eq!(r, "Session: ${AGENTRS_SESSION_ID}");
    }
    ...
    fn tc_9_1_multiple_placeholder_types() {
        let r = substitute_arguments(
            "cd ${AGENTRS_SKILL_DIR} && run $ARGUMENTS[0] with $ARGUMENTS",
            Some("alpha beta"),
            &[],
            Some("/skills/foo"),
            None,
