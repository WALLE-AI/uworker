...
// AC-5: $ARGUMENTS, $0, ${AGENTRS_SKILL_DIR} are correctly substituted
// ---------------------------------------------------------------------------
...
async fn tc_e2e_2_inline_variable_substitution() {
    ...
    assert!(
        ...
        "${{AGENTRS_SKILL_DIR}} should be replaced with skill root, got: {result}"
    );
    ...
    assert!(
        !result.contains("${AGENTRS_SKILL_DIR}"),
        "${{AGENTRS_SKILL_DIR}} literal should not remain, got: {result}"
    );
}
...
// AC-14: legacy command files from .agentrs/commands/ are loaded as SkillDefinition
// ---------------------------------------------------------------------------
...
async fn tc_e2e_11_legacy_commands_loaded() {
    ...
    // load_all_skills looks for .agentrs/commands/ relative to cwd.
    let commands_dir = tmp.path().join(".agentrs").join("commands");
    ...
    let skill = skills
        ...
        .expect("legacy-cmd should be loaded from flat .md file in .agentrs/commands/");
