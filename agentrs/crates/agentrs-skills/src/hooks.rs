...
use agentrs_config::hooks::{HookDef, HooksConfig};
...
pub fn parse_skill_hooks(
    ...
    if source == SkillSource::Mcp {
        tracing::warn!(target: "agentrs_skills", skill = %skill_name, "hooks ignored for MCP source");
        return None;
    }
    ...
    let obj = match raw.as_object() {
        Some(o) => o,
        None => {
            tracing::warn!(target: "agentrs_skills", skill = %skill_name, "hooks_raw is not a JSON object, ignoring");
            return None;
        }
    };
    ...
    for (event_key, matchers_val) in obj {
        let target = match event_key.as_str() {
            ...
            other => {
                tracing::warn!(target: "agentrs_skills", skill = %skill_name, event = %other, "unknown hook event, skipping");
                continue;
            }
        };
        ...
        let matchers = match matchers_val.as_array() {
            Some(a) => a,
            None => {
                tracing::warn!(target: "agentrs_skills", skill = %skill_name, event = %event_key, "hook event value is not an array, skipping");
                continue;
            }
        };
        ...
        for matcher_entry in matchers {
            ...
            for hook in hooks_arr {
                ...
                match hook["type"].as_str() {
                    Some("command") => {}
                    Some(other) => {
                        tracing::warn!(target: "agentrs_skills", skill = %skill_name, hook_type = %other, "unsupported hook type, skipping");
                        continue;
                    }
                    None => {
                        tracing::warn!(target: "agentrs_skills", skill = %skill_name, "hook missing type field, skipping");
                        continue;
                    }
                }
                ...
                let command = match hook["command"].as_str() {
                    Some(c) => c.to_string(),
                    None => {
                        tracing::warn!(target: "agentrs_skills", skill = %skill_name, "command-type hook missing command field, skipping");
                        continue;
                    }
                };
