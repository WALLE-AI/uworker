...
async fn tc_4_5_cwd_used() {
    use agentrs_config::shell::ShellKind;

    let tmp = std::env::temp_dir();
    let shell = agentrs_config::shell::default_shell();
...
async fn tc_windows_skill_shell_powershell_stdout_preserved() {
    let result = run("before !`Write-Output agentrs_skill_stdout_probe` after")
        .await
        .unwrap();

    assert!(
        result.contains("agentrs_skill_stdout_probe"),
