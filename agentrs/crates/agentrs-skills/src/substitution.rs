...
/// 5. Skill directory: `${AGENTRS_SKILL_DIR}` → `skill_root`
/// 6. Session ID: `${AGENTRS_SESSION_ID}` → `session_id`
...
pub fn substitute_arguments(
    ...
    if let Some(root) = skill_root {
        result = result.replace("${AGENTRS_SKILL_DIR}", root);
    }

    // 6. ${AGENTRS_SESSION_ID}
    if let Some(sid) = session_id {
        result = result.replace("${AGENTRS_SESSION_ID}", sid);
    }
