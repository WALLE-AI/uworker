...
pub fn default_log_dir() -> PathBuf {
    ...
    {
        dirs::home_dir()
            .map(|h| h.join("Library").join("Logs").join("agentrs"))
            .unwrap_or_else(|| PathBuf::from("agentrs/logs"))
    }
    #[cfg(target_os = "linux")]
    {
        dirs::state_dir()
            .map(|d| d.join("agentrs").join("logs"))
            .unwrap_or_else(|| {
                dirs::home_dir()
                    .map(|h| h.join(".local").join("state").join("agentrs").join("logs"))
                    .unwrap_or_else(|| PathBuf::from("agentrs/logs"))
            })
    }
