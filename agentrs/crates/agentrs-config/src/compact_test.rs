...
"#;
    ...
    fn default_compaction_is_safe() {
        let cfg = CompactConfig::default();
        assert_eq!(cfg.compaction, agentrs_compact::CompactLevel::Safe);
    }
    ...
    fn toml_compaction_level_override() {
        ...
        let cfg: CompactConfig = toml::from_str(toml_str).unwrap();
        assert_eq!(cfg.compaction, agentrs_compact::CompactLevel::Full);
    }
    ...
    fn toml_compaction_off() {
        ...
        let cfg: CompactConfig = toml::from_str(toml_str).unwrap();
        assert_eq!(cfg.compaction, agentrs_compact::CompactLevel::Off);
    }
