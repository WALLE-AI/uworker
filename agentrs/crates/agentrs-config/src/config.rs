...
fn resolve_api_key(cli_key: Option<&str>, config_key: Option<&str>, provider: ProviderType) -> anyhow::Result<String> {
    ...
    anyhow::bail!(
        "No API key found. Provide via --api-key, config file, environment variable \
         (API_KEY, ANTHROPIC_API_KEY, or OPENAI_API_KEY), or run 'agentrs auth login'."
    )
}
...
///
/// - Linux:   `~/.config/agentrs`
/// - macOS:   `~/Library/Application Support/agentrs`
/// - Windows: `%APPDATA%\agentrs`
pub fn app_config_dir() -> Option<PathBuf> {
    dirs::config_dir().map(|d| d.join("agentrs"))
}
...
pub fn global_config_path() -> PathBuf {
    app_config_dir()
        .unwrap_or_else(|| PathBuf::from("agentrs"))
        .join("config.toml")
}

fn project_config_path() -> PathBuf {
    PathBuf::from(".agentrs.toml")
}
        ...
        Ok(content) => match toml::from_str::<ConfigFile>(&content) {
            Ok(config) => {
                let compact_context_window = match toml::from_str::<ConfigFieldPresence>(&content) {
                    ...
                    Err(error) => {
                        tracing::warn!(target: "agentrs_config", path = %path.display(), %error, "failed to inspect config field presence");
                        None
                    }
                };
                LoadedConfigFile {
            ...
            Err(error) => {
                tracing::warn!(target: "agentrs_config", path = %path.display(), %error, "failed to parse config file");
...
fn merge_config_files(global: ConfigFile, project: ConfigFile) -> ConfigFile {
    ...
    // Presentation only: the project file wins whenever it deviates from the
    // default, matching how the other single-value sections merge.
    let tui = if project.tui != TuiConfig::default() {
        project.tui
    } else {
        global.tui
    };
...
pub fn init_config() -> anyhow::Result<()> {
    ...
    if path.exists() {
        tracing::info!(target: "agentrs_config", path = %path.display(), "config file already exists");
        return Ok(());
    }
    ...
    tracing::info!(target: "agentrs_config", path = %path.display(), "config file created");
    Ok(())
}

const DEFAULT_CONFIG_TEMPLATE: &str = r#"# agentrs configuration
...
# OAuth settings (for `agentrs auth login` with Claude.ai account)
# [auth]
...
# dir = "~/Library/Logs/agentrs"    # log directory (default: platform-specific)
