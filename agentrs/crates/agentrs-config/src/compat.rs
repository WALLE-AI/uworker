...
pub struct TransportCompat {
    ...
    /// Vendor-specific top-level fields merged into every projected request
    /// body. Lets deployments reach endpoint features the neutral
    /// `LlmRequest` does not model — for example vLLM's
    /// `chat_template_kwargs = { enable_thinking = true }`.
    ///
    /// Keys that the projector already produced are left untouched, so this
    /// can only add fields, never silently rewrite the wire contract.
    /// Default: None (nothing merged).
...
impl TransportCompat {
    fn merge(defaults: Self, user: Self) -> Self {
        Self {
            ...
            extra_body: merge_extra_body(defaults.extra_body, user.extra_body),
        }
    }
}

/// Merge user `extra_body` over the preset one key by key, so a preset entry
/// survives unless the user overrides that exact key.
fn merge_extra_body(
    defaults: Option<Map<String, Value>>,
    user: Option<Map<String, Value>>,
) -> Option<Map<String, Value>> {
    match (defaults, user) {
        (Some(mut defaults), Some(user)) => {
            defaults.extend(user);
            Some(defaults)
        }
        (defaults, user) => user.or(defaults),
    }
}
...
impl ProviderCompat {
    ...
    /// Vendor-specific fields to merge into the projected request body.
    pub fn extra_body(&self) -> Option<&Map<String, Value>> {
        self.transport.extra_body.as_ref().filter(|extra| !extra.is_empty())
    }
