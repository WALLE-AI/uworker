...
//   Source: crates/aion-agent/src/context_usage.rs @ f711174
//   Copied: 2026-09-01   Modified: yes
//   Changes: aion_types → agentrs_types；`DateTime<Utc>` 换成 `Timestamp`、
//            `Utc::now()` 换成调用方传入——内核经 Clock port 拿时刻，
//            直接读真实时钟会让 `cargo test --workspace` 不再确定
...
use super::estimate::{estimate_tokens_from_tool_image, estimate_tokens_from_tool_result};
...
impl ContextState {
    pub fn replace_with_provider_usage(&mut self, context_usage: u64) {
        self.context_usage = context_usage;
        self.source = ContextUsageSource::ProviderExact;
        self.touch();
    }

    pub fn replace_with_local_estimate(&mut self, context_usage: u64) {
        self.context_usage = context_usage;
        self.source = ContextUsageSource::LocalProjected;
        self.touch();
    }

    pub fn add_local_estimate(&mut self, tokens: u64) {
    ...
    pub fn record_microcompact(&mut self) {
...
#[derive(Debug, Clone)]
/// 一份上下文用量快照。字段供宿主呈现，本 crate 自身不读。
#[allow(dead_code, reason = "快照字段供宿主读取")]
...
pub fn estimate_content_tokens(content: &[ContentBlock]) -> u64 {
    content.iter().fold(0_u64, |total, block| {
        let tokens = match block {
            ...
            ContentBlock::ToolUse { id, name, input, extra } => {
                ...
                estimate_chars(id.as_str().len() + name.len() + input.to_string().len() + extra_len)
            }
...
pub fn estimate_tool_definitions_tokens(tools: &[ToolDef]) -> u64 {
...
fn estimate_tool_tokens(tool: &ToolDef) -> u64 {
    estimate_chars(tool.name.len() + tool.description.len() + tool.parameters.to_string().len())
}
