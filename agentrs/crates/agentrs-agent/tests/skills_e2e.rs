...
//! `.agentrs/skills/` and `.agentrs/commands/` layout, then exercises the full
...
use agentrs_agent::context::{SystemPromptCache, build_system_prompt};
use agentrs_agent::skill_tool::SkillTool;
use agentrs_agent::skills::loader::load_all_skills;
use agentrs_agent::skills::permissions::SkillPermissionChecker;
use agentrs_agent::skills::types::SkillMetadata;
...
/// Create a project-like temp directory with `.git` marker and `.agentrs/skills/` + `.agentrs/commands/`.
...
fn make_project() -> (TempDir, PathBuf) {
    ...
    // Skills directory
    let skills_dir = root.join(".agentrs").join("skills");
    fs::create_dir_all(&skills_dir).unwrap();

    // Commands directory (legacy)
    let commands_dir = root.join(".agentrs").join("commands");
...
async fn e2_legacy_commands_discovered() {
    ...
    println!("E2 PASS: legacy command 'legacy-cmd' discovered from .agentrs/commands/");
}
...
// ---------------------------------------------------------------------------
// E8: Full SkillTool execution (db:migrate with $ARGUMENTS + ${AGENTRS_SKILL_DIR})
// ---------------------------------------------------------------------------
...
async fn e8_full_execution() {
    ...
    assert!(
        !result.content.contains("${AGENTRS_SKILL_DIR}"),
        "E8 FAIL: ${{AGENTRS_SKILL_DIR}} not expanded. Got: {}",
        result.content
    );
    println!("E8 PASS: full execution with $ARGUMENTS and ${{AGENTRS_SKILL_DIR}} substitution");
}
