...
use agentrs_agent::confirm::ToolConfirmer;
use agentrs_agent::engine::AgentEngine;
use agentrs_agent::orchestration::execute_tool_calls;
...
use agentrs_config::config::{Config, ProviderType, SessionConfig, ToolsConfig};
use agentrs_config::hooks::HooksConfig;
use agentrs_mcp::config::McpConfig;
use agentrs_providers::create_provider;
...
fn openai_config(api_key: &str) -> Config {
    Config {
        ...
        tools: ToolsConfig {
            auto_approve: true,
            allow_list: vec![],
            skills: agentrs_config::config::SkillsPermissionConfig::default(),
        },
...
impl agentrs_tools::Tool for FixedOutputTool {
    fn name(&self) -> &str {
        &self.name
    }
    ...
    fn category(&self) -> agentrs_protocol::events::ToolCategory {
        agentrs_protocol::events::ToolCategory::Info
    }
