...
use agentrs_agent::error::AgentError;
use agentrs_agent::output::OutputSink;
use agentrs_agent::output::terminal::TerminalSink;
use agentrs_agent::session::SessionManager;
...
async fn tc_2_6_05_session_save_after_compact() {
    ...
    let has_boundary = session.messages.iter().any(|m| {
        m.content.iter().any(|b| {
            matches!(b, agentrs_types::message::ContentBlock::Text { text } if text.contains("[Conversation compacted]"))
        })
    });
...
async fn tc_2_6_02_micro_before_auto_execution_order() {
    ...
    let captured: Arc<Mutex<Option<Vec<agentrs_types::message::Message>>>> = Arc::new(Mutex::new(None));
    let capture_ref = captured.clone();

    struct OrderProvider {
        regular_count: Mutex<usize>,
        captured: Arc<Mutex<Option<Vec<agentrs_types::message::Message>>>>,
    }
    ...
    let cleared_count = msgs
        ...
        .filter(|b| {
            matches!(
                b,
                agentrs_types::message::ContentBlock::ToolResult { content, .. }
                    if content == agentrs_agent::compact::micro::CLEARED_TOOL_RESULT
            )
        })
        .count();
