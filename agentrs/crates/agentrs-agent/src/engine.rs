...
use agentrs_compact::CompactLevel;
...
pub struct AgentEngine {
    ...
    msg_id: String,
    /// Turn id for the NEXT run, supplied by the host (e.g. AgentrsCore passes
...
impl AgentEngine {
    ...
    /// (e.g. AgentrsCore) pass them in so the session-side anchor matches their
    ...
    async fn run_inner(&mut self, content_blocks: Vec<ContentBlock>, msg_id: &str) -> Result<AgentResult, AgentError> {
        ...
        loop {
            ...
            if let Some(limit) = guards.turn_budget_reached() {
                ...
                warn!(target: "agentrs_agent", limit, "stopping agent run at turn budget");
    ...
    fn record_turn_usage(&mut self, turn_usage: &TokenUsage) -> bool {
        ...
        if context_usage == 0 {
            debug!(target: "agentrs_agent", "provider omitted turn usage; retaining local context projection");
            return false;
        }
    ...
    async fn run_compaction(&mut self) -> Result<(), AgentError> {
        ...
        if should_compact {
            info!(target: "agentrs_agent", context_tokens = self.compact_state.last_input_tokens, "context compaction triggered");
...
impl AgentEngine {
    ...
    pub async fn run_stop_hooks(&self) {
        if let Some(hook_engine) = &self.hooks {
            ...
            for msg in messages {
                info!(target: "agentrs_agent", hook_message = %msg, "stop hook output");
            }
        }
    }
}
