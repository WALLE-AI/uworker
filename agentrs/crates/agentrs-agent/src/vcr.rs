...
impl VcrLayer {
    ...
    pub fn save(&self) -> anyhow::Result<()> {
        if let VcrMode::Record(path) = &self.mode {
            ...
            tracing::info!(target: "agentrs_agent", interactions = cassette.interactions.len(), path = %path.display(), "vcr cassette saved");
        }
        Ok(())
    }
}
...
impl Drop for VcrLayer {
    fn drop(&mut self) {
        if let Err(e) = self.save() {
            tracing::warn!(target: "agentrs_agent", error = %e, "failed to save vcr cassette");
        }
    }
}
...
fn load_cassette(path: &Path) -> anyhow::Result<Cassette> {
    ...
    tracing::info!(target: "agentrs_agent", name = %cassette.name, interactions = cassette.interactions.len(), "vcr cassette loaded");
    Ok(cassette)
}
