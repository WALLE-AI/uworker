...
mod tests_set_config {
    ...
    use agentrs_config::compat::ReasoningCompat;
    use agentrs_config::config::{CliArgs, CompactContextWindowSource, Config};
    ...
    fn make_resolved_openai_engine(model: &str, project_toml: &str) -> super::AgentEngine {
        ...
        if !project_toml.is_empty() {
            std::fs::write(directory.path().join(".agentrs.toml"), project_toml).unwrap();
        }
...
"#,
    ...
    fn set_config_enables_thinking() {
        ...
        match &engine.thinking {
            Some(agentrs_types::llm::ThinkingConfig::Enabled { budget_tokens }) => {
    ...
    fn set_config_disables_thinking() {
        let mut engine = make_engine("m");
        engine.thinking = Some(agentrs_types::llm::ThinkingConfig::Enabled { budget_tokens: 8000 });
        let changes = engine.apply_config_update(None, None, Some("disabled".into()), None, None, None);
        match &engine.thinking {
            Some(agentrs_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_thinking_budget_only_updates_existing_enabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(agentrs_types::llm::ThinkingConfig::Enabled { budget_tokens: 5000 });
    ...
    fn set_config_thinking_budget_ignored_when_disabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(agentrs_types::llm::ThinkingConfig::Disabled);
        let changes = engine.apply_config_update(None, None, None, Some(20000), None, None);
        match &engine.thinking {
            Some(agentrs_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_thinking_applies_when_capability_is_false() {
        ...
        assert!(matches!(
            engine.thinking,
            Some(agentrs_types::llm::ThinkingConfig::Enabled { budget_tokens: 10000 })
        ));
    }
...
mod tests_phase6 {
    ...
    use agentrs_tools::registry::ToolRegistry;
    use agentrs_types::llm::{LlmEvent, LlmRequest};
    use agentrs_types::skill_types::{ContextModifier, EffortLevel};
...
mod tests_compact {
    ...
    use agentrs_tools::registry::ToolRegistry;
    use agentrs_types::llm::{LlmEvent, LlmRequest};
    use agentrs_types::message::{ContentBlock, ImageInputCapability, ImageUrl, Message, Role, StopReason, TokenUsage};
    use chrono::Utc;
    use serde_json::json;
    use tempfile::tempdir;
    use tokio::sync::mpsc;
...
mod tests_plan_mode {
    ...
    use agentrs_tools::registry::ToolRegistry;
    use agentrs_types::llm::{LlmEvent, LlmRequest};
    use agentrs_types::skill_types::{ContextModifier, PlanModeTransition};
...
mod tests_handle_command {
    ...
    #[tokio::test]
    async fn run_treats_agentrs_files_marker_as_plain_text() {
        let mut engine = make_engine_with_provider(Arc::new(SingleResponseProvider));
        let input = "discuss [[AGENTRS_FILES]] and file.txt";
...
mod tests_tool_policy_enforcement {
    ...
    use agentrs_tools::registry::ToolRegistry;
    use agentrs_types::llm::{LlmEvent, LlmRequest};
    use agentrs_types::message::{ContentBlock, ImageInputCapability};
    use agentrs_types::tool::ToolResult;
    use async_trait::async_trait;
    use serde_json::{Value, json};
