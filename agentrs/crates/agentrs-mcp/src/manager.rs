...
impl McpManager {
        ...
        Fut: Future<Output = Result<McpServer, McpError>>,
    {
        ...
        while let Some((name, result)) = pending.next().await {
            match result {
                Ok(server) => {
                    tracing::info!(target: "agentrs_mcp", server = %name, tools = server.tools.len(), resources = server.supports_resources, "mcp server connected");
                    servers.insert(name, server);
                }
                Err(e) => {
                    // Non-fatal: continue with other servers
                    tracing::warn!(target: "agentrs_mcp", server = %name, error = %e, "mcp server connection failed");
                }
            }
        }
    ...
    pub async fn shutdown(&self) {
        for (name, server) in &self.servers {
            if let Err(e) = server.transport.close().await {
                tracing::warn!(target: "agentrs_mcp", server = %name, error = %e, "error closing mcp server");
            }
        }
    }
